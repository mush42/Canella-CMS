...
api_bp = CanellaModule('api', __name__, url_prefix='/api')
api = Api(api_bp)
ma = Marshmallow(app)
cors = CORS(app, resources='/api/*', origins=['http://localhost:3000'], supports_credentials=True, max_age=1)

class CanellaResourceType(MethodViewType):
...
class CanellaResource(Resource, metaclass=CanellaResourceType):
    """
    ...
    def get(self, pk=None, attr=None):
        ...
        return self.schema_dump(paginator.items, many=True), 200, {'X-Total-Count': len(paginator.items)}
...
class CanellaSchemaOpts(SchemaOpts):
    def __init__(self, meta):
        super(CanellaSchemaOpts, self).__init__(meta)
        self.json_module = json

class CanellaModelSchema(ma.ModelSchema):
    OPTIONS_CLASS = CanellaSchemaOpts
